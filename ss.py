    ...
    with xpc.XPlaneConnect() as client:
        while True:
            ...
            if len(edge_coordinates) >= 2:
                ...
                pitch_PID.update(desired_pitch)
                ...
                new_throttle_ctrl = 1  # Assuming throttle values range from 0 to 1, where 1 is full throttle

                # Send control input to X-Plane
                ctrl = [new_ele_ctrl, new_ail_ctrl, 0.0, new_throttle_ctrl]
                client.sendCTRL(ctrl)

                 # Calculate center of horizon line
                center_horizon_line = ((horizon_line[0] + horizon_line[2]) // 2, (horizon_line[1] + horizon_line[3]) // 2)

        # Draw center of horizon line
                cv2.circle(frame, center_horizon_line, 5, (0, 255, 0), -1)

        # Calculate center of the frame
                center_frame = (w // 2, h // 2)

        # Draw center of the frame
                cv2.circle(frame, center_frame, 5, (0, 255, 0), -1)
